...
dat <- as.data.frame(compartilhamento3)
...
## alternativa frequentista
library(mlogit)
library(arm)
compartilhamento3_aux <- compartilhamento3 %>%
  mutate(decisao_compartilhamento = as.character(decisao_compartilhamento),
         decisao_compartilhamento = if_else(grepl("1", decisao_compartilhamento), "ataque_1", 
                                            if_else(grepl("2", decisao_compartilhamento), "ataque_2", "not_publish")),
         decisao_compartilhamento = as.factor(decisao_compartilhamento)) %>%
  dplyr::select(decisao_compartilhamento, nivel_identificacao_recode, score_narcisism, score_id_nac, liberal_conservador, ladder, genero, idade) %>%
  mutate(nivel_identificacao_recode_normal = rescale(nivel_identificacao_recode))

compartilhamento3_wide <- mlogit.data(compartilhamento3_aux, varying=NULL, choice = "decisao_compartilhamento", shape= "wide")
model1 <- mlogit(decisao_compartilhamento ~ 1 | nivel_identificacao_recode_normal, data= compartilhamento3_wide, reflevel = "not_publish")
summary(model1)

model2 <- mlogit(decisao_compartilhamento ~ 1 | nivel_identificacao_recode_normal + score_narcisism + score_id_nac, 
                 data= compartilhamento3_wide, reflevel = "ataque_1")
summary(model2)
invlogit(0.12611 + -0.41591*2)
invlogit(0.12611)
#